...
User can only see their own githubs activities */
  useEffect(() => {
      api
        ...
        .then((response) => {
          if (response.data.github.length!=0 && response.data.github.match("[^/]+(?!.*/)")){
            let name = response.data.github.split("com/")[1];
            setGithubName(name);
            
            axios.get(`https://api.github.com/users/${name}/events`)
                .then((response)=>{
...
useEffect(() => {
      ...
      .then((response) => {
        const authorList = response.data.items;
            for (let author of authorList) {
                if (author.github.length!=0 && author.github.match("[^/]+(?!.*/)")){
  ...
  return (
    ...
    <div className='homepage'>
       <Row>
        <Col md={5}><h1>GitHub Activites</h1></Col>
        ...
        <div>
            ...
            myGithubActivities.map((activity, i)=>(
                ...
                <Card className="post-card" key={i}>
                        <Card.Header>
                            <div className="post-author" onClick={()=> routeChange(githubName)}>
                                <div className="profile-pic-post">
                                    <img src={activity.actor.avatar_url} alt="profilePic" />
                                </div>
                                <div className="post-author-name">{activity.actor.login}</div>
                            </div>
                        ...
                        <Card.Body>
                            ...
                            <Card.Text>
                                ...
                                <p>Commit: {typeof activity.payload.commits!== 'undefined'?
                                activity.payload.commits[0].message
                                    :<p> No commit message!</p>}</p>
