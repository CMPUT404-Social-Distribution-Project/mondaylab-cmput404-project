    ...
    const sendPost = async () => {
        ...
        if (!imagePost) {
            ...
            .then((response) => {
                ...
                if (post.unlisted) {
                    setURI(`${window.location.protocol}//${window.location.host}/authors/${user_id}/posts/${response.data.uuid}`)
                    setShowURI(true)
                } else {
                    props.onHide();
                }
            ...
            .catch((error) => {
                alert(`Something went wrong posting the image post! \n Error: ${error.response.data}`)
        ...
        <>
                    ...
                    <Form>
                        ...
                        <Modal.Body>
                            <Form.Group className="title">
                                <Form.Control label="title" name="title" type="text" placeholder="Title" value={post.title}
                                    onChange={(e) => {
                            ...
                            <Form.Group>
                                <Form.Control as="textarea" className="body" type="content" placeholder="Write your Post..." value={post.content}
                                    onChange={(e) => {
                                        setPost({
                                            ...post,
                        ...
                        {imagePreview === null ? <></> : <img className= "image-preview" src={imagePreview} alt="postImage"/>}
                        <Modal.Footer>
                            <div className="image-upload-container">
                                <FaImage className='image-upload'  onClick={handleImageClick}/>
                                <form onSubmit={(e) => onFileSubmit(e)} onChange={(e) => onImageChange(e)} >
                                    <input type="file" name="file" accept='.jpeg, .png, .jpg' onChange={imageUpload} ref={hiddenFileInput} style={{display:"none"}}/>
                                </form>
                                {imagePreview === null ? <></> : <Button className="remove-image" onClick={removeImage}>Remove Image</Button>}
                            </div>
            ...
            <div>
                <Modal
                    ...
                    <Modal.Body>
                        <Form.Group>
                            <Form.Label style={{ color: "var(--teal)", paddingBottom: "3%" }}>Shareable URI:</Form.Label>
                            <Form.Control readOnly size="lg" defaultValue={uri} />
                        </Form.Group>
                    </Modal.Body>
                    <Modal.Footer>
                        <Button className="ok-button" size="lg" onClick={() => {setShowURI(false); props.onHide()}}> OK! </Button>
                    </Modal.Footer>
                </Modal>
            </div>
        </>
